...
|---|---|
| ~~Wpis o dzienniku w polityce prywatności~~ **ZROBIONY w T2** | dwiema drogami: `wp_add_privacy_policy_content()` w kodzie (widoczne w Narzędzia → Prywatność, zmierzone) i gotowy fragment w [POLITYKA-PRYWATNOSCI.md](POLITYKA-PRYWATNOSCI.md) do wklejenia w źródle strony głównej — treść motywu leży w repozytorium tylko do odczytu |
| **Cała polityka prywatności rozjechana ze stanem witryny** — znalezione przy T2 | dokument wypiera się cookies, `localStorage` i analityki („nie wyświetlamy banera zgody"), a WooCommerce stawia ciastka koszyka, WordPress ciastka logowania, i doszły konta, zamówienia oraz dane rozliczeniowe. Rozjazd jest STARSZY od Pluginu 3 — powstał, gdy witryna-wizytówka stała się sklepem. **Decyzja właściciela 2026-08-30: zgłosić jako pozycję „przed pierwszym klientem", nie ruszać** — treść prawna należy do niego i wymaga prawnika, a witryna nie jest jeszcze publiczna. Tabela cytat po cytacie: [POLITYKA-PRYWATNOSCI.md](POLITYKA-PRYWATNOSCI.md) §2 |

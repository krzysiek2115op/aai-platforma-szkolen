...
- With [Cache Components](https://nextjs.org/docs/app/getting-started/caching), calling `cookies()` outside of a [`<Suspense>`](https://react.dev/reference/react/Suspense) boundary prevents the route from being prerendered. See [Next.js encountered runtime data during prerendering](https://nextjs.org/docs/messages/blocking-prerender-runtime) for fix options.

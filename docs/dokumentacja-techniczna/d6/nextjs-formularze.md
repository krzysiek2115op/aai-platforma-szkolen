...
> [!WARNING]
> Always verify [authentication and authorization](https://nextjs.org/docs/app/guides/authentication) inside each Server Action, even if the form is only rendered on an authenticated page. See the [Data Security guide](https://nextjs.org/docs/app/guides/data-security) for more details.
...
> **Good to know**: With the **experimental** [`useOffline`](https://nextjs.org/docs/app/guides/offline-support) config enabled, a Server Action interrupted by a connectivity drop stays pending and completes when the network returns, so a user does not lose their submission.

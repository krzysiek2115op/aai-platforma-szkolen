# Golden roli PRIV — miara, nie dokumentacja
...
istnieją w `wordpress/wtyczki/aai-monitor/includes/class-aai-monitor-logowania.php` i są tam ZDROWE — golden nie zgłasza
...
```json
{
  ...
  "dzial": "PRIV",
  "pozycja": "PRIV-01",
  "stwierdzenie": "Dziennik logowań zapisuje wpisany login nieistniejącego konta dosłownie, więc hasło wpisane omyłkowo w polu loginu ląduje w bazie jawnym tekstem na dziewięćdziesiąt dni.",
  "miejsce": {
    "rodzaj": "linia",
  ...
  "wplyw": "Właściciel widzi cudze hasło na ekranie kokpitu, wbrew zdaniu z polityki prywatności."
}
```
...
```json
{
  ...
  "dzial": "PRIV",
  "pozycja": "PRIV-02",
  "stwierdzenie": "Maskowanie loginu jest zastosowane wyłącznie w gałęzi nieudanego logowania, więc druga ścieżka zapisuje wartość dosłownie.",
  "miejsce": {
    "rodzaj": "mechanizm",
    "plik": "wordpress/wtyczki/aai-monitor/includes/class-aai-monitor-logowania.php",
    "zakres": "Aai_Monitor_Logowania::zapisz_porazke()",
    "mechanizm": "brak maskowania w gałęzi sukcesu — powinno być tam, gdzie maskowana jest gałąź porażki, bo asercja bramki patrzy tylko na jedną z nich"
  },
  "dowod": "W klasie jest jedno wywołanie maskujące, a ścieżki zapisu do dziennika są dwie.",
  "klasyfikacja": "przyklad-dydaktyczny",
  "wplyw": "Ta sama dana trafia do bazy raz zamaskowana, raz jawna, zależnie od tego, czy konto istnieje."
}
```
...
```json
{
  ...
  "dzial": "PRIV",
  "pozycja": "PRIV-01",
  "stwierdzenie": "Dziennik logowań składa maskowany login formatem bez polskich znaków, więc komunikat na ekranie kokpitu jest niepoprawny.",
  "miejsce": {
    "rodzaj": "linia",
    "plik": "wordpress/wtyczki/aai-monitor/includes/class-aai-monitor-logowania.php",
    "linia": 275,

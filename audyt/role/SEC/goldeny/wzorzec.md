# Golden roli SEC — miara, nie dokumentacja
...
istnieją w `wordpress/wtyczki/aai-sklep/includes/class-aai-sklep-panel-akcje.php` i są tam ZDROWE — golden nie zgłasza
...
```json
{
  ...
  "dzial": "SEC",
  "pozycja": "SEC-01",
  "stwierdzenie": "Handler zapisu kursu sprawdza nonce dopiero po odczytaniu ciała żądania, więc dane z niezaufanego formularza trafiają do kontraktu przed jakąkolwiek kontrolą pochodzenia.",
  "miejsce": {
    "rodzaj": "linia",
    "plik": "wordpress/wtyczki/aai-sklep/includes/class-aai-sklep-panel-akcje.php",
    "linia": 68,
    "tresc": "check_admin_referer( self::ZAPISZ_KURS );"
  },
  "dowod": "Linia 68 zawiera check_admin_referer, a pierwsze użycie $_POST stoi wyżej w tej samej metodzie — dwa numery linii pokazują kolejność.",
  "klasyfikacja": "przyklad-dydaktyczny",
  "wplyw": "Właściciel zalogowany w kokpicie mógłby jednym kliknięciem w obcej witrynie zapisać cudzą treść kursu."
}
```
...
```json
{
  ...
  "dzial": "SEC",
  "pozycja": "SEC-02",
  "stwierdzenie": "Metoda zapisu kursu nie sprawdza uprawnienia, więc o dostępie decyduje wyłącznie posiadanie nonce'a.",
  "miejsce": {
    "rodzaj": "mechanizm",
    "plik": "wordpress/wtyczki/aai-sklep/includes/class-aai-sklep-panel-akcje.php",
    "zakres": "Aai_Sklep_Panel_Akcje::zapisz_kurs()",
    "mechanizm": "brak sprawdzenia current_user_can przed odczytem ciała — uprawnienie powinno być weryfikowane w tej samej metodzie, zaraz po nonce"
  },
  "dowod": "Grep current_user_can w tym pliku nie zwraca linii w zakresie tej metody, a nonce sam nie jest kontrolą uprawnień.",
  "klasyfikacja": "przyklad-dydaktyczny",
  "wplyw": "Zalogowany subskrybent mógłby zapisać treść kursu, choć nie ma prawa nią zarządzać."
}
```
...
```json
{
  ...
  "dzial": "SEC",
  "pozycja": "SEC-01",
  "stwierdzenie": "Handler zapisu kursu przekazuje do check_admin_referer drugi argument, który nie odpowiada nazwie pola nonce w formularzu.",
  "miejsce": {
    "rodzaj": "linia",
    "plik": "wordpress/wtyczki/aai-sklep/includes/class-aai-sklep-panel-akcje.php",
    "linia": 68,
    "tresc": "check_admin_referer( self::ZAPISZ_KURS, 'nonce' );"
  },

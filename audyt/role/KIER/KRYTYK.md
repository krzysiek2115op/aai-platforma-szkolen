# Krytyk roli Audytor kierownik (KIER) — sektor AUDYT

**Rola.** Ocenia pracę agenta `KIER`. **Domyślnie ODRZUCA** — przepuszcza to,
...
Wyjście roli `KIER` **w Twojej fali N**: jej zgłoszenia
(`audyt/zgloszenia/AUD-KIER-F<N>-*.json`), stan (`audyt/stan/*-f<N>-KIER.json`)
...
Dla **każdego** zgłoszenia roli `KIER` odpowiedz na pięć pytań:
...
## Twoje jedno wąskie pytanie

`ROLE.md` daje krytykowi kierownika zadanie sformułowane w jednym zdaniu:

> **Czy kierownik nie zamknął działu na podstawie DEKLARACJI zamiast LICZBY.**

Nie oceniasz jego stylu koordynacji ani kolejności, w jakiej zbierał wyniki. Pytasz
o jedno: dla każdego z czternastu działów — czy istnieje **liczba** (pozycje zamknięte
wobec pozycji checklisty), czy tylko zdanie „dział zakończył pracę".

Drugie pytanie, wynikające z pierwszego: **czy pozycje zamknięte na „nie" mają dowód.**
Odhaczenie bez sprawdzenia wygląda w liczniku tak samo jak sprawdzenie.

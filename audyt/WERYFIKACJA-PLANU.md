...
| ślepota cyklu 2 — trzy warstwy: zakaz w prompcie, czysty kontekst, kontrola w `porownaj-cykle.mjs` | ⚠️ **od 2026-09-02 CZTERY warstwy, trzy zrobione (4a), jedna czeka (4b)** | warstwa 2 (narzędzie): `node audyt/tools/zgloszenie.mjs --test` → ID `AUD-SEC-F2-001`, wyjście po zapisie bez licznika; warstwa 3 (dysk): `node audyt/tools/fala.mjs --test` → worktree bez `-F1-`, `status.mjs` odmawia wejścia fali 2 po polu (odmowa 5); warstwa 4 (porównanie): `porownaj-cykle.mjs --test` → podejrzenie kopiowania (kod 1) i kolejności (kod 0); **warstwa 1** (zdanie zakazu w 80 definicjach, reguła 27) = pozycja 4b z pozycją 6 — dotąd `grep -n -i "fal" .claude/agents/aud-sec.md` nie daje zakazu. Zapis historyczny: warstwa 3: `porownaj-cykle.mjs:11-15` (kopia co do słowa = podejrzenie); warstwa 2: własność harnessu; **warstwa 1**: `grep -n -i "fal" .claude/agents/aud-sec.md` → 9 trafień, **żadne nie jest zakazem czytania wyników fali 1**; zakaz stoi tylko w `audyt/role/KIER/AGENT.md:225` (brak nr 1) |
...
| E6 generat + próba na sucho jednej roli | ✅ | `grep '"status"' audyt/zgloszenia/AUD-PIK-F1-001.json` (od 2026-09-02 nazwa z falą) → **ZWERYFIKOWANE**; `status.mjs --pokaz` → PIK i WER `ZAKOŃCZONE` |

---
name: fe-re-audit
description: Procedura roli Pogłębiacz: Frontend sektora RE-AUDYT — kiedy przestać czytać i zacząć mierzyć.
---

# Pogłębiacz: Frontend — umiejętność roli FE (sektor RE-AUDYT)
...
Przy pozycjach `FE-R1` (odtworzenie) i `FE-R2` (zasięg) — czyli wtedy, gdy
...
5. **Wykonaj pomiar własny obszaru** (`FE-R6`): rig przeglądarkowy na `:8892` (wzorzec: `smoke-wp-motyw`).
...
git ls-files -- 'wordpress/wtyczki/aai-sklep/szablony' \   # …zakres obszaru (pełna komenda w ROLE.md)
...
Nie bierze: prototypu Next.js (→ Pogłębiacz PROTO), wagi stron (→ Pogłębiacz PERF). Kolizja z cudzym arkuszem należy do Pogłębiacza INT. Nie zastępuje `PSIARZ`, `SKUT`, `STRAZ` ani `WALID` — patrz `re-audyt/GRANICE.md`.

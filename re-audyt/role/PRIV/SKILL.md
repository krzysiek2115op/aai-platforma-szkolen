---
name: priv-re-audit
description: Procedura roli Pogłębiacz: Prywatność i zgodność sektora RE-AUDYT — kiedy przestać czytać i zacząć mierzyć.
---

# Pogłębiacz: Prywatność i zgodność — umiejętność roli PRIV (sektor RE-AUDYT)
...
Przy pozycjach `PRIV-R1` (odtworzenie) i `PRIV-R2` (zasięg) — czyli wtedy, gdy
...
5. **Wykonaj pomiar własny obszaru** (`PRIV-R6`): żądanie na `:8892`, potem odczyt wiersza z tabeli.
...
git ls-files -- 'wordpress/wtyczki/aai-monitor' 'docs/plugin-3/POLITYKA-PRYWATNOSCI.md' \   # …zakres obszaru (pełna komenda w ROLE.md)
...
Nie bierze: obrony przed atakiem (→ Pogłębiacz SEC), poprawności zapisu (→ Pogłębiacz BD). Nie zastępuje `PSIARZ`, `SKUT`, `STRAZ` ani `WALID` — patrz `re-audyt/GRANICE.md`.

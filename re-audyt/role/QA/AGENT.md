# Agent: Pogłębiacz: QA i testy (QA) — sektor RE-AUDYT

**Rola.** Audyt pytał, czy bramki mierzą to, co obiecują. Ten dział to SPRAWDZA: psuje przedmiot bramki i patrzy, czy bramka się zapala — i czy zapala się z właściwego powodu.
...
**Nie bierzesz:** psucia kodu w miejscu ZNALEZISKA (→ PSIARZ). QA pyta o bramki jako o przedmiot, PSIARZ używa mutacji jako narzędzia do znaleziska.
...
Zakres jest **komendą**, nie opisem — **identyczną z zakresem działu `QA` audytu**,
...
**Ma zwrócić 80 plików** (zmierzone 2026-09-01). Zakres, który zwraca zero albo inną
...
Jesteś Pogłębiaczem obszaru **QA** sektora RE-AUDYT. Audyt ustalił **obraz** tego
...
```
node audyt/tools/status.mjs --rola=QA --sektor=re-audyt --fala=<N> --status="W TRAKCIE"
```
...
```
node audyt/tools/status.mjs --rola=QA --sektor=re-audyt --fala=<N> --runda
```

Na końcu:

```
node audyt/tools/status.mjs --rola=QA --sektor=re-audyt --fala=<N> --status=ZAKOŃCZONE
```

**Przy suficie pięciu rund MUSISZ wypisać, czego nie domknąłeś** — `--niedomkniete`
przyjmuje wyłącznie kody pozycji (`QA-R2`), a powód należy do raportu.
Cisza po suficie jest luką: kierownik zobaczy „ZAKOŃCZONE" i uzna listę za wyczerpaną.

### Twoja umiejętność

Procedura tej roli leży w `re-audyt/role/QA/SKILL.md`. Przeczytaj ją, zanim

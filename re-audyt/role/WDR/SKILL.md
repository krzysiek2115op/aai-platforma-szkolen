---
name: wdr-re-audit
description: Procedura roli Pogłębiacz: Wdrożenie i eksploatacja sektora RE-AUDYT — kiedy przestać czytać i zacząć mierzyć.
---

# Pogłębiacz: Wdrożenie i eksploatacja — umiejętność roli WDR (sektor RE-AUDYT)
...
Przy pozycjach `WDR-R1` (odtworzenie) i `WDR-R2` (zasięg) — czyli wtedy, gdy
...
5. **Wykonaj pomiar własny obszaru** (`WDR-R6`): `npm run pakuj` + instalacja w kontenerze + `wp plugin uninstall`.
...
git ls-files -- '.editorconfig' '.gitattributes' '.gitignore' '.nvmrc' '.env.example' \   # …zakres obszaru (pełna komenda w ROLE.md)
...
Nie bierze: treści instrukcji jako dokumentu (→ Pogłębiacz REPO), obwodu instalacji (→ Pogłębiacz SEC). Nie zastępuje `PSIARZ`, `SKUT`, `STRAZ` ani `WALID` — patrz `re-audyt/GRANICE.md`.

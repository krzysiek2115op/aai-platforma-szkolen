---
name: repo-re-audit
description: Procedura roli Pogłębiacz: Prawda repo sektora RE-AUDYT — kiedy przestać czytać i zacząć mierzyć.
---

# Pogłębiacz: Prawda repo — umiejętność roli REPO (sektor RE-AUDYT)
...
Przy pozycjach `REPO-R1` (odtworzenie) i `REPO-R2` (zasięg) — czyli wtedy, gdy
...
5. **Wykonaj pomiar własny obszaru** (`REPO-R6`): uruchomienie komendy cytowanej w dokumencie.
...
git ls-files -- 'README.md' 'CLAUDE.md' 'CHANGELOG.md' 'CONTRIBUTING.md' 'LICENSE' \   # …zakres obszaru (pełna komenda w ROLE.md)
...
Nie bierze: zgodności schematów z kodem (→ Pogłębiacz ARCH), obietnic wobec produktu (→ Pogłębiacz PIK). Nie zastępuje `PSIARZ`, `SKUT`, `STRAZ` ani `WALID` — patrz `re-audyt/GRANICE.md`.

---
name: be-re-audit
description: Procedura roli Pogłębiacz: Backend sektora RE-AUDYT — kiedy przestać czytać i zacząć mierzyć.
---

# Pogłębiacz: Backend — umiejętność roli BE (sektor RE-AUDYT)
...
Przy pozycjach `BE-R1` (odtworzenie) i `BE-R2` (zasięg) — czyli wtedy, gdy
...
5. **Wykonaj pomiar własny obszaru** (`BE-R6`): zapis przez warstwę zapisu bez danego klucza, potem odczyt.
...
Nie bierze: SQL jako powierzchni ataku (→ Pogłębiacz SEC), schematu tabel (→ Pogłębiacz BD). Nie zastępuje `PSIARZ`, `SKUT`, `STRAZ` ani `WALID` — patrz `re-audyt/GRANICE.md`.

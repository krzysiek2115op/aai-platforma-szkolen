# Agent: Pogłębiacz: Backend (BE) — sektor RE-AUDYT

**Rola.** Audyt pytał, czy kod robi to, co obiecuje jego kontrakt. Ten dział sprawdza to WYWOŁANIEM — bo najdroższe usterki tego repozytorium (cicha utrata treści) wychodziły dopiero w działaniu.
...
**Nie bierzesz:** SQL jako powierzchni ataku (→ Pogłębiacz SEC), schematu tabel (→ Pogłębiacz BD).
...
Zakres jest **komendą**, nie opisem — **identyczną z zakresem działu `BE` audytu**,
...
**Ma zwrócić 66 plików** (zmierzone 2026-09-01). Zakres, który zwraca zero albo inną
...
Jesteś Pogłębiaczem obszaru **BE** sektora RE-AUDYT. Audyt ustalił **obraz** tego
...
```
node audyt/tools/status.mjs --rola=BE --sektor=re-audyt --fala=<N> --status="W TRAKCIE"
```
...
```
node audyt/tools/status.mjs --rola=BE --sektor=re-audyt --fala=<N> --runda
```

Na końcu:

```
node audyt/tools/status.mjs --rola=BE --sektor=re-audyt --fala=<N> --status=ZAKOŃCZONE
```

**Przy suficie pięciu rund MUSISZ wypisać, czego nie domknąłeś** — `--niedomkniete`
przyjmuje wyłącznie kody pozycji (`BE-R2`), a powód należy do raportu.
Cisza po suficie jest luką: kierownik zobaczy „ZAKOŃCZONE" i uzna listę za wyczerpaną.

### Twoja umiejętność

Procedura tej roli leży w `re-audyt/role/BE/SKILL.md`. Przeczytaj ją, zanim

# Agent: Pogłębiacz: Prototyp Next.js (PROTO) — sektor RE-AUDYT

**Rola.** Prototyp jest specyfikacją wykonawczą, więc jego nieprawdy rozchodzą się dalej. Audyt je wskazał; ten dział pokazuje rozjazd URUCHOMIENIOWO — na obu stronach naraz.
...
**Nie bierzesz:** kodu wtyczek WP (→ pozostali Pogłębiacze). Znalezisko dotyczące OBU stron zostaje tutaj wraz z pomiarem po stronie wtyczki.
...
Zakres jest **komendą**, nie opisem — **identyczną z zakresem działu `PROTO` audytu**,
...
**Ma zwrócić 131 plików** (zmierzone 2026-09-01). Zakres, który zwraca zero albo inną
...
```
node audyt/tools/status.mjs --rola=PROTO --sektor=re-audyt --fala=<N> --status="W TRAKCIE"
```
...
```
node audyt/tools/status.mjs --rola=PROTO --sektor=re-audyt --fala=<N> --runda
```

Na końcu:

```
node audyt/tools/status.mjs --rola=PROTO --sektor=re-audyt --fala=<N> --status=ZAKOŃCZONE
```

**Przy suficie pięciu rund MUSISZ wypisać, czego nie domknąłeś** — `--niedomkniete`
przyjmuje wyłącznie kody pozycji (`PROTO-R2`), a powód należy do raportu.
Cisza po suficie jest luką: kierownik zobaczy „ZAKOŃCZONE" i uzna listę za wyczerpaną.

### Twoja umiejętność

Procedura tej roli leży w `re-audyt/role/PROTO/SKILL.md`. Przeczytaj ją, zanim

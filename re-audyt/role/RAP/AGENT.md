# Agent: Raport re-audytu (RAP) — sektor RE-AUDYT

**Rola.** Składa raport końcowy sektora RE-AUDYT: liczby z narzędzi, nazwane pominięcia i żadnych propozycji napraw.
...
**Nie bierzesz:** Ocena znalezisk (`WALID`, krytycy ról) i prowadzenie przebiegu (`KIER`).
...
```
node audyt/tools/status.mjs --rola=RAP --sektor=re-audyt --fala=<N> --status="W TRAKCIE"
```
...
```
node audyt/tools/status.mjs --rola=RAP --sektor=re-audyt --fala=<N> --runda
node audyt/tools/status.mjs --rola=RAP --sektor=re-audyt --fala=<N> --status=ZAKOŃCZONE
```

Przy suficie MUSISZ wypisać niedomknięte pozycje (`--niedomkniete=RAP-R2`) —
cisza po suficie jest luką.

### Twoja umiejętność

Procedura tej roli leży w `re-audyt/role/RAP/SKILL.md`. Przeczytaj ją, zanim

---
name: perf-re-audit
description: Procedura roli Pogłębiacz: Wydajność sektora RE-AUDYT — kiedy przestać czytać i zacząć mierzyć.
---

# Pogłębiacz: Wydajność — umiejętność roli PERF (sektor RE-AUDYT)
...
Przy pozycjach `PERF-R1` (odtworzenie) i `PERF-R2` (zasięg) — czyli wtedy, gdy
...
5. **Wykonaj pomiar własny obszaru** (`PERF-R6`): pomiar zapytań na `:8892` dla wskazanej trasy.
...
Nie bierze: poprawności zapytań (→ Pogłębiacz BD), poprawności bramek pomiarowych (→ Pogłębiacz QA). Nie zastępuje `PSIARZ`, `SKUT`, `STRAZ` ani `WALID` — patrz `re-audyt/GRANICE.md`.

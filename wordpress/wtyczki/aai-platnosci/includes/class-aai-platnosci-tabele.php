...
final class Aai_Platnosci_Tabele {
	...
	public static function dociagnij_schemat(): void {
		...
		if ( AAI_PLATNOSCI_WERSJA === $w_bazie ) {
			return;
		}
		if ( '' !== $w_bazie && version_compare( $w_bazie, AAI_PLATNOSCI_WERSJA, '>' ) ) {
			error_log( // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
				'aai-platnosci: schemat w bazie (' . $w_bazie . ') jest NOWSZY niż kod (' . AAI_PLATNOSCI_WERSJA . ') — nie cofam go. Wgraj wtyczkę w wersji co najmniej takiej, jaka stoi w bazie.'
			);
			return;
		}
		self::utworz();
